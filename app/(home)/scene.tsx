...
const cubicInOut = (t: number): number => {
	return t < 0.5 ? 4 * t * t * t : 0.5 * (2 * t - 2) ** 3 + 1;
};
...
function useDarkMode() {
	...
	useEffect(() => {
		const checkDarkMode = () => {
			...
			setIsDarkMode(
				hasDarkClass ||
					(!htmlElement.classList.contains("light") && systemPrefersDark),
			);
		};
...
const Scene = forwardRef<HTMLDivElement>((props, ref) => {
	...
	return (
		<div ref={ref} className="w-full h-full">
			<WebGPUCanvas camera={{ position: [0, 0, 5], fov: 50 }} shadows>
