...
export const FAQ = () => (
  <Section className="divide-border grid divide-y divide-dashed lg:grid-cols-2 lg:divide-x lg:divide-y-0">
    ...
    <Accordion
      type="single"
      collapsible
      className="divide-border w-full divide-dashed"
    >
      {faq.map((item, index) => (
        <AccordionItem
          ...
          value={`index-${index}`}
        >
          <AccordionTrigger className="hover:bg-background-muted data-[state=open]:bg-background-muted rounded-none px-4 hover:no-underline">
            {item.question}
          </AccordionTrigger>
