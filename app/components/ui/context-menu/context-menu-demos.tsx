		...
		switch (type) {
			case 'pdf':
				return (
					<FileTextIcon
						aria-hidden="true"
						className="mr-3 h-5 w-5 text-foreground-subtle"
					/>
				);
			case 'image':
				return (
			...
			<div className="flex flex-col space-y-2">
				{files.map((file, index) => (
					<ContextMenu key={index}>
						<ContextMenuTrigger>
							<div className="flex cursor-context-menu items-center gap-3 rounded-md border border-none border-border bg-background p-3 transition-all duration-200 ease-in-out dark:border-solid">
