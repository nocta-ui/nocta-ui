  ...
  return (
    <div className={`not-prose ${containerClassName}`}>
      ...
      <div className="relative">
        {leftIcon && (
          <div className={iconVariants({ position: 'left', size, disabled: !!disabled })}>
            {leftIcon}
          </div>
        )}
        
        <input
          className={cn(inputVariants({ variant, size, hasLeftIcon, hasRightIcon }), className)}
          disabled={disabled}
          {...props}
        />
        
        {rightIcon && (
          <div className={iconVariants({ position: 'right', size, disabled: !!disabled })}>
            {rightIcon}
          </div>
        )}
      </div>
      ...
      {!displayErrorMessage && successMessage && (
        <p className={messageVariants({ type: 'success' })}>
          {successMessage}
        </p>
      )}

      {helperText && (
        <p className={messageVariants({ type: 'helper' })}>
          {helperText}
        </p>
      )}
    </div>
