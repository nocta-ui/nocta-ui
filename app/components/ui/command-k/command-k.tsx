...
export const CommandK: React.FC<CommandKProps> = ({
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (!requiresMeta && !requiresCtrl) {
        ...
        modifierOk = (requiresMeta ? e.metaKey : true) && (requiresCtrl ? e.ctrlKey : true);
      }
  ...
  }, [highlightedIndex, flatSelectable.length]);
  ...
  const onKeyNav = (e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case "ArrowUp":
        e.preventDefault();
        setHighlightedIndex((i) => (i - 1 + flatSelectable.length) % flatSelectable.length);
        break;
      case "Enter":
        e.preventDefault();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent size={size} showClose={false} className={cn("w-full p-0", className)}>
        <div className="px-2 pt-3" ref={searchContainerRef}>
          <div className="relative">
            <Input
              ...
              className="w-full pr-12 border-none focus-visible:ring-0 focus-visible:ring-offset-0 !shadow-none"
              leftIcon={
            ...
            <div className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-fit">
              <span className={kbdClass()}>{shortcutHint}</span>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          className={cn(
            ...
            "border-t border-border-muted/30",
            listClassName,
          )}
          role="listbox"
        ...
        >
          {flatSelectable.length === 0 ? (
            <div className="px-3 py-6 text-sm text-foreground-subtle text-center">
              {emptyMessage}
            </div>
          ) : (
            Array.from(grouped.entries()).map(([group, entries]) => (
              <div key={group} className="px-2">
                <div className={groupHeaderClass()}>{group}</div>
                <div className="mt-1 flex flex-col gap-1">
                  {entries.map((entry, idxInGroup) => {
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {entry.item.description && (
                            <div className="truncate text-xs text-foreground-subtle mt-0.5">
                              {entry.item.description}
                            </div>
                          )}
                        </div>
                        {entry.item.shortcut && entry.item.shortcut.length > 0 && (
                          <div className="flex items-center gap-1 ml-2">
                            {(() => {
                              const sc = entry.item.shortcut ?? [];
                              const hasShift = sc.some((t) => /^(shift|⇧)$/i.test(t.trim()));
                              const letters = sc
                                .filter((t) => /^[a-z0-9]$/i.test(t.trim()))
                                .map((t) => t.trim().toUpperCase());
                              const tokens = [isMac ? "⌘" : "Ctrl", ...(hasShift ? ["⇧"] : []), ...letters];
                              return tokens.map((tok, i) => (
                                <span key={i} className={kbdClass()}>{tok}</span>
                              ));
                            })()}
                          </div>
                        )}
