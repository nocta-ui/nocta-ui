...
const toastContainerVariants = cva(
	'not-prose pointer-events-auto fixed rounded-lg border border-none shadow-2xl backdrop-blur-lg transition-all duration-200 ease-in-out will-change-transform dark:border-solid',
	{
		variants: {
			...
			variant: {
				default:
					'overflow-hidden border-border bg-background-muted text-foreground',
				...
				>
					<div className={cn(toastContentVariants({ variant }))}>
						<button
							...
							onClick={handleClose}
							className={cn(
								'absolute top-2 right-2 cursor-pointer rounded-md p-1 text-foreground-subtle transition-colors duration-200 hover:text-foreground-muted focus-visible:ring-1 focus-visible:ring-ring/50 focus-visible:outline-none',
							)}
						...
						<div className="p-4 pr-8">
							...
							{description && (
								<div
									id={descriptionId}
									className="text-sm leading-relaxed text-foreground-muted/80 opacity-80"
								>
									{description}
								</div>
							)}
							{action && (
								<div className="mt-3">
									<button
										...
										className="inline-flex cursor-pointer items-center justify-center rounded-md bg-foreground px-3 py-1.5 text-sm font-medium text-background shadow-sm transition-all duration-200 hover:bg-foreground-muted focus-visible:ring-ring/50"
									>
										{action.label}
									</button>
								</div>
