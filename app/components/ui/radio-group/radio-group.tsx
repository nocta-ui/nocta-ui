...
const indicatorVariants = cva(
	'flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full border border-border transition-colors duration-150 ease-in-out',
	{
		variants: {
			checked: {
...
const dotVariants = cva(
	'rounded-full bg-foreground transition-transform duration-150 ease-in-out',
	{
		variants: {
			checked: {
...
>(({ value, label, description, disabled, className, ...props }, ref) => {
	...
	return (
		<label
			...
			className={cn(
				'group relative flex items-start gap-2 cursor-pointer select-none rounded-md transition-colors duration-150 ease-in-out',
