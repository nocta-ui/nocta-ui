...
const chatVariants = cva(
	[
		'relative border border-none border-border bg-background dark:border-solid',
	...
	],
	{
		variants: {
			variant: {
				...
				system:
					'mx-auto bg-background-muted text-center text-xs text-foreground-subtle',
			},
		},
		defaultVariants: {
			variant: 'user',
...
const inputVariants = cva(
	[
		...
		'not-prose placeholder:text-foreground-subtle',
...
export const TypingIndicator: React.FC<TypingIndicatorProps> = ({
	...
	return (
		...
		>
			{showAvatars && (
				<div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-background-muted text-xs font-medium text-foreground-muted">
			...
			<div className="flex w-full flex-col items-start gap-1">
				<div className="not-prose w-fit max-w-[80%] rounded-lg bg-background-muted px-3 py-2 text-sm text-foreground transition-all duration-200 ease-in-out">
					<div className="flex items-center gap-2">
						<span className="text-xs text-foreground-muted">
							{getTypingText()}
						</span>
						<div className="flex gap-1">
							<div className="h-1.5 w-1.5 animate-bounce rounded-full bg-foreground-muted [animation-delay:-0.3s]" />
							<div className="h-1.5 w-1.5 animate-bounce rounded-full bg-foreground-muted [animation-delay:-0.15s]" />
							<div className="h-1.5 w-1.5 animate-bounce rounded-full bg-foreground-muted" />
						</div>
					</div>
				</div>
			</div>
...
export const ChatInput: React.FC<ChatInputProps> = ({
	...
	return (
		<div
			className={cn(
				'not-prose border-t border-border/60 bg-background-muted/50 p-4 dark:bg-background-muted/30',
				className,
			)}
			{...props}
		>
			...
			{maxLength && (
				<div className="mt-1 text-right text-xs text-foreground-subtle dark:text-foreground-subtle">
