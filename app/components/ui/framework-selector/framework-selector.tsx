          ...
          <Link href={`/docs/${framework.id}`} key={framework.id} className="no-underline">
            ...
            >
              <div className='flex h-full gap-4 w-full flex-col items-center justify-center border bg-nocta-200 dark:bg-nocta-900 transition-colors duration-300 rounded-md p-8'>
                {framework.logo}
                <span className='text-lg font-medium text-nocta-600 dark:text-nocta-400'>{framework.name}</span>
              </div>
            </button>
          </Link>
        ))}
