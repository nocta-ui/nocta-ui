		...
		<button
			...
			className={cn(
				buttonVariants({
					...
					size: 'sm',
					className:
						'relative font-mono text-foreground/35 cursor-pointer bg-popover-muted shadow-sm card-highlight border-border',
				}),
				className,
			)}
			{...props}
...
export function SearchDialogContent({
	...
	return (
		<DialogContent
			...
			{...props}
			className={cn(
				'fixed card-highlight left-1/2 top-4 md:top-[calc(50%-250px)] z-50 w-[calc(100%-1rem)] max-w-screen-sm -translate-x-1/2 rounded-lg border border-border bg-popover text-foreground/70 shadow-2xl data-[state=closed]:animate-fd-dialog-out data-[state=open]:animate-fd-dialog-in',
...
export function SearchDialogListItem({
	...
	return (
		<button
			...
			className={cn(
				'relative select-none px-2.5 py-2 text-start text-sm rounded-md cursor-pointer',
