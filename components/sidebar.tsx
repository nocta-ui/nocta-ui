      ...
      <Presence present={open}>
        {({ present }) => (
          <aside
            ...
            data-state={state}
            className={cn(
              'border-border bg-background data-[state=open]:animate-fd-sidebar-in data-[state=closed]:animate-fd-sidebar-out fixed inset-y-0 end-0 z-40 flex w-[85%] max-w-[380px] flex-col border-s text-[15px] shadow-lg',
              !present && 'invisible',
              className
            )}
          >
...
export function SidebarSeparator(props: ComponentProps<'p'>) {
  return (
    <p
      {...props}
      className={cn(
        'text-foreground-subtle mb-1.5 inline-flex items-center gap-2 px-2 ps-(--sidebar-item-offset) !text-xs tracking-wider uppercase empty:mb-0 [&_svg]:size-4 [&_svg]:shrink-0',
        props.className
      )}
    >
      {props.children}
    ...
    <CollapsibleContent
      ...
      className={cn(
        ...
        level === 1 && [
          ...
          "**:data-[active=true]:before:bg-foreground **:data-[active=true]:before:absolute **:data-[active=true]:before:inset-y-2.5 **:data-[active=true]:before:start-2.5 **:data-[active=true]:before:w-px **:data-[active=true]:before:content-['']",
        ],
        props.className
      )}
      style={
