			...
			<Presence present={open}>
				{({ present }) => (
					<aside
						...
						data-state={state}
						className={cn(
							"fixed text-[15px] flex flex-col shadow-lg border-border border-s end-0 inset-y-0 w-[85%] max-w-[380px] z-40 bg-background data-[state=open]:animate-fd-sidebar-in data-[state=closed]:animate-fd-sidebar-out",
							!present && "invisible",
							className,
						)}
					>
...
export function SidebarFooter(props: ComponentProps<"div">) {
	return (
		<div
			{...props}
			className={cn(
				"flex flex-col border-t border-dashed p-4 pt-2 mx-2",
				props.className,
			)}
		>
			{props.children}
