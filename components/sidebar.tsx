...
const itemVariants = cva(
	'cursor-pointer relative flex flex-row items-center gap-2 rounded-md p-1.5 mb-1 ps-(--sidebar-item-offset) text-start [overflow-wrap:anywhere] text-foreground/70 [&_svg]:size-4 [&_svg]:shrink-0 focus-visible:ring-1 focus-visible:ring-offset-1 focus-visible:outline-none focus-visible:ring-offset-ring-offset/50 focus-visible:ring-ring/50',
	{
		variants: {
			active: {
...
export function SidebarContent(props: ComponentProps<'aside'>) {
	...
	return (
		<aside
			...
			className={cn(
				'fixed bg-card border-r border-fd-border card-highlight top-(--fd-sidebar-top) bottom-(--fd-sidebar-margin) left-0 z-20 flex flex-col items-end text-sm transition-[top,opacity,translate,width] duration-450 ease-smooth *:w-(--fd-sidebar-width) max-md:hidden rtl:right-(--removed-body-scroll-bar-size,0) rtl:left-auto',
				collapsed && [
			...
			<Presence present={open}>
				{({ present }) => (
					<aside
						...
						data-state={state}
						className={cn(
							'fixed inset-y-0 end-0 z-40 flex w-[85%] max-w-[380px] flex-col border-s border-border bg-card rounded-l-lg text-[15px] shadow-lg data-[state=closed]:animate-fd-sidebar-out data-[state=open]:animate-fd-sidebar-in',
							!present && 'invisible',
							className,
						)}
					>
...
export function SidebarFooter(props: ComponentProps<'div'>) {
	return (
		<div
			{...props}
			className={cn(
				'mx-2 flex flex-col border-t border-fd-border p-4 pt-2',
				props.className,
			)}
		>
			{props.children}
