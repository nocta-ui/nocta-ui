		...
		<Popover open={open} onOpenChange={setOpen}>
			{item && (
				<PopoverTrigger
					{...props}
					className={cn(
						'flex items-center gap-2 rounded-lg border bg-foreground/50 p-2 text-start text-foreground/45 duration-150 ease-basic transition-colors hover:bg-card-muted data-[state=open]:bg-card-muted data-[state=open]:text-foreground/45',
						props.className,
					)}
				>
					{item}
				...
				{options.map((item) => {
					...
					return (
						<Link
							...
							{...item.props}
							className={cn(
								'flex items-center gap-2 rounded-lg p-1.5 transition-all duration-150 ease-basic hover:bg-card-muted hover:text-foreground/45',
