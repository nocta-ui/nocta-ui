...
export function PageTOCPopoverTrigger(props: ComponentProps<'button'>) {
  ...
  return (
    <CollapsibleTrigger
      {...props}
      className={cn(
        'text-foreground-muted flex h-(--fd-tocnav-height) w-full items-center gap-2.5 px-4 py-2.5 text-start text-sm focus-visible:outline-none md:px-6 [&_svg]:size-4',
        props.className
      )}
    >
      <ProgressCircle
...
function FooterItem({ item, index }: { item: Item; index: 0 | 1 }) {
  ...
  return (
    <Link
      href={item.url}
      className={cn(
        'bg-background hover:bg-background-muted/50 hover:text-foreground-muted flex flex-col gap-2 rounded-lg border p-4 text-sm transition-colors @max-lg:col-span-full',
        index === 1 && 'text-end'
      )}
    >
      <div
...
export function PageBreadcrumb({
  ...
  return (
    <div
      {...props}
      className={cn(
        'text-foreground-muted flex items-center gap-1.5 text-sm',
        props.className
      )}
    >
      {items.map((item, i) => {
