...
export function PageTOCPopoverTrigger(props: ComponentProps<"button">) {
	...
	return (
		<CollapsibleTrigger
			{...props}
			className={cn(
				"flex w-full h-(--fd-tocnav-height) items-center text-sm text-foreground-muted gap-2.5 px-4 py-2.5 text-start focus-visible:outline-none [&_svg]:size-4 md:px-6",
				props.className,
			)}
		>
			<ProgressCircle
...
function FooterItem({ item, index }: { item: Item; index: 0 | 1 }) {
	...
	return (
		<Link
			href={item.url}
			className={cn(
				"flex flex-col gap-2 rounded-lg border p-4 text-sm transition-colors bg-background hover:bg-background-muted/50 hover:text-foreground-muted @max-lg:col-span-full",
				index === 1 && "text-end",
			)}
		>
			<div
...
export function PageTOC(props: ComponentProps<"div">) {
	...
	return (
		<div
			...
			{...props}
			className={cn(
				"fixed bottom-0 pt-8 pb-2 right-0 pr-(--removed-body-scroll-bar-size,0) max-xl:hidden",
				props.className,
			)}
			style={{
				...props.style,
		...
		>
			<div className="flex h-full w-(--fd-toc-width) max-w-full flex-col pe-4 px-8">
				{props.children}
			</div>
		</div>
	);
