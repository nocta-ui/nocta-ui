...
export function PageTOCTitle(props: ComponentProps<'h2'>) {
  return (
    <h3
      {...props}
      className={cn(
        'text-foreground-muted inline-flex items-center gap-1.5 text-sm',
        props.className
      )}
    >
...
export function PageArticle(props: ComponentProps<'article'>) {
  return (
    <article
      {...props}
      className={cn(
        'border-border bg-background flex h-svh w-full min-w-0 flex-col gap-4 overflow-y-auto border-r border-l border-dashed px-4 pt-8 md:mx-auto md:px-6',
        props.className
      )}
    >
      {props.children}
