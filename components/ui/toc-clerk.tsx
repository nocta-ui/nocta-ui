...
export default function ClerkTOCItems({
  ...
  if (items.length === 0)
    return (
      <div className="bg-foreground text-foreground-muted rounded-lg border p-3 text-xs">
        {text.tocNoHeadings}
      </div>
    )
...
function TOCItem({
  ...
  return (
    <Primitive.TOCItem
      ...
      className="prose text-foreground-muted/75 hover:text-foreground-muted data-[active=true]:text-foreground relative py-1.5 text-sm [overflow-wrap:anywhere] transition-colors first:pt-0 last:pb-0"
    >
      {offset !== upperOffset ? (
        <svg
