    ...
    <Navbar>
      ...
      <div className="flex flex-1 flex-row items-center justify-end lg:gap-1.5">
        <SearchOnly>
          ...
          <LargeSearchToggle className="!bg-background w-full max-w-[240px] max-lg:hidden" />
        </SearchOnly>
        <ThemeToggle />
            ...
            <div className="-ms-1.5 flex flex-row items-center gap-1.5 max-sm:mt-2">
              ...
              {i18n ? (
                <LanguageToggle>
                  ...
                  <ChevronDownIcon
                    aria-hidden="true"
                    className="text-foreground-muted size-3"
                  />
                </LanguageToggle>
              ) : null}
            </div>
  ...
  if (item.type === 'menu') {
    const children = item.items.map((child, j) => {
      ...
      return (
        <NavbarMenuLink key={j.toString()} href={child.url} {...rest}>
          {banner ??
            (child.icon ? (
              <div className="bg-background-muted w-fit rounded-md border p-1 [&_svg]:size-4">
                {child.icon}
              </div>
            ) : null)}
    ...
    return (
      <NavbarMenu>
        ...
        <NavbarMenuContent className="border-border container border-dashed sm:border-x">
          {children}
        </NavbarMenuContent>
      </NavbarMenu>
    )
