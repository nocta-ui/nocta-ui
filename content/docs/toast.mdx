...
The toast component requires GSAP for animations:
...
```tsx title="Toast Provider Setup"
...
<DocsTabs defaultValue="preview">
  ...
  <DocsTab value="code" title="Code">
    ```tsx title="Custom Toast Duration"
    const showToast = () => {
      toast({
        title: 'Quick Toast',
...
```tsx title="Custom Toast Styling"
toast({
  title: 'Custom Toast',
