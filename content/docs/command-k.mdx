---
title: Command Palette
description: A command palette modal (Cmd/Ctrl+K) for quick actions and navigation with search, grouping, and full keyboard accessibility
---

import { CommandK } from '@/app/components/ui/command-k';
import { BasicCommandKDemo } from '@/app/components/ui/command-k/command-k-demos';
import { DocsTabs, DocsTab } from "@/app/components/ui/docs-tab"
import { Step, Steps } from "fumadocs-ui/components/steps"

## Installation

<Steps>
  <Step>
    Install the Command Palette component using the nocta-ui CLI:

    ```bash lineNumbers title="Terminal"
    npx nocta-ui add command-k
    ```
  </Step>
  <Step>
    Import the component:

    ```tsx lineNumbers title="Import"
    import { CommandK } from '@/components/ui/command-k';
    ```
  </Step>
</Steps>

## Basic Usage

The Command Palette provides a fast, searchable modal for navigating and triggering actions using the keyboard.

<DocsTabs defaultValue="preview">
  <DocsTab value="preview" lineNumbers title="Preview">
    <BasicCommandKDemo />
  </DocsTab>
  <DocsTab value="code" lineNumbers title="Code">
    ```tsx lineNumbers title="Basic Command Palette"
    import { useState } from 'react';
    import { CommandK, type CommandKItem } from '@/components/ui/command-k';

     const items: CommandKItem[] = [
    { label: "Search docs", group: "Navigation", description: "Go to documentation", icon: IconSearch, shortcut: ["G", "D"] },
    { label: "Open settings", group: "Navigation", description: "Application preferences", icon: IconCog, shortcut: ["G", "S"] },
    { label: "New file", group: "Actions", description: "Create an untitled file", icon: IconFile, shortcut: ["N", "F"] },
    { label: "New folder", group: "Actions", description: "Create a folder", icon: IconFolder, shortcut: ["N", "D"] },
  ];

    export default function Example() {
      const [open, setOpen] = useState(false);
      return (
        <>
          <button onClick={() => setOpen(true)}>Open Command Palette</button>
          <CommandK
            items={items}
            open={open}
            onOpenChange={setOpen}
            onSelect={(it) => {
              // eslint-disable-next-line no-alert
              alert(`Selected: ${it.label}`);
            }}
          />
        </>
      );
    }
    ```
  </DocsTab>
</DocsTabs>

## Item Structure

Define your palette entries using the `CommandKItem` interface:

```tsx lineNumbers title="CommandKItem"
export type CommandKItem = {
  id?: string;            // Optional unique id (auto-generated if missing)
  label: string;          // Display label
  group?: string;         // Optional grouping header (e.g., 'Navigation')
  description?: string;   // Secondary text under the label
  keywords?: string[];    // Additional searchable terms
  icon?: React.ReactNode; // Optional icon element
  shortcut?: string[];    // Optional visual shortcut hints (e.g., ['G','H'])
  disabled?: boolean;     // Disable selection
  action?: () => void;    // Action invoked on selection
};
```

## Keyboard Shortcuts

- Open: platform-default `⌘K` (macOS) / `Ctrl+K` (Windows/Linux)
- Navigate: `↑/↓`
- Confirm: `Enter`
- First/Last: `Home` / `End`
- Close: `Escape`

### Customizing the open hotkey

```tsx lineNumbers title="Custom Hotkey"
// Require Cmd+P on macOS, Ctrl+P on Windows/Linux
<CommandK hotkey={{ key: 'p' }} />

// Explicit modifier (always Cmd)
<CommandK hotkey={{ key: 'k', metaKey: true }} />

// Explicit modifier (always Ctrl)
<CommandK hotkey={{ key: 'k', ctrlKey: true }} />
```

## Controlled vs Hotkey

You can control visibility with `open`/`onOpenChange` or rely solely on the global hotkey.

```tsx lineNumbers title="Controlled"
const [open, setOpen] = useState(false);

<CommandK open={open} onOpenChange={setOpen} items={items} />
```

```tsx lineNumbers title="Hotkey Only"
// No local state — palette opens with Cmd/Ctrl+K
<CommandK items={items} />
```

## Accessibility

Built on top of the `Dialog` and `Input` primitives for consistent UX and a11y:

- Proper roles: `role="listbox"`/`role="option"` for results
- Focus trap and Escape to close via Dialog
- Visual focus indicators and high-contrast colors
- Screen reader-friendly labels and states

## API Reference

### CommandK

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `items` | `CommandKItem[]` | — | List of entries to render |
| `open` | `boolean` | — | Controlled open state |
| `defaultOpen` | `boolean` | `false` | Initial open state |
| `onOpenChange` | `(open: boolean) => void` | — | Callback when open state changes |
| `placeholder` | `string` | `'Type a command or search...'` | Search input placeholder |
| `emptyMessage` | `string` | `'No results'` | Message when no results match |
| `closeOnAction` | `boolean` | `true` | Close palette after selection |
| `size` | `'sm' \| 'md' \| 'lg' \| 'xl' \| 'full'` | `'xl'` | Dialog size variant |
| `className` | `string` | `''` | Extra classes for DialogContent wrapper |
| `listClassName` | `string` | `''` | Extra classes for results list |
| `autoFocus` | `boolean` | `true` | Focus search input when opened |
| `hotkey` | `{ key: string; metaKey?: boolean; ctrlKey?: boolean; }` | `{ key: 'k' }` | Open shortcut; when no modifier specified, uses platform default (⌘/Ctrl) |
| `onSelect` | `(item: CommandKItem) => void` | — | Called on item selection |

### CommandKItem

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `id` | `string` | auto | Optional unique ID |
| `label` | `string` | — | Display label |
| `group` | `string` | `'General'` | Group header label |
| `description` | `string` | `''` | Secondary description text |
| `keywords` | `string[]` | `[]` | Additional searchable terms |
| `icon` | `React.ReactNode` | — | Optional icon |
| `shortcut` | `string[]` | `[]` | Visual shortcut hints |
| `disabled` | `boolean` | `false` | Disable selection |
| `action` | `() => void` | — | Action executed on select |

