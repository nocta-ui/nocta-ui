...
Once the CLI copies components into `packages/ui`, rebuild the shared UI package so the compiled output stays in sync:

```bash title="Terminal"
bun run --filter ui build
# or keep everything hot-reloading in watch mode
bun run --filter ui dev
```

Back in your application workspace, consume the freshly published exports as normal:
