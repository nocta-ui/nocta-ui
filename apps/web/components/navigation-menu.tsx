...
>(({ className, children, ...props }, ref) => (
	<Primitive.Trigger
		ref={ref}
		className={cn('data-[state=open]:bg-card-muted', className)}
		{...props}
	>
		{children}
	</Primitive.Trigger>
...
>(({ className, ...props }, ref) => (
	<Primitive.Content
		ref={ref}
		className={cn(
			'absolute inset-x-0 top-0 fd-scroll-container max-h-[80svh] overflow-auto data-[motion=from-end]:animate-fd-enterFromRight data-[motion=from-start]:animate-fd-enterFromLeft data-[motion=to-end]:animate-fd-exitToRight data-[motion=to-start]:animate-fd-exitToLeft',
			className,
		)}
		{...props}
	/>
...
>(({ className, ...props }, ref) => (
	<div ref={ref} className="flex w-full justify-center">
		<Primitive.Viewport
			{...props}
			className={cn(
				'relative h-(--radix-navigation-menu-viewport-height) w-full origin-[top_center] overflow-hidden transition-[width,height] duration-300 ease-smooth data-[state=closed]:animate-fd-nav-menu-out data-[state=open]:animate-fd-nav-menu-in',
				className,
			)}
		/>
	</div>
